...
## 2026-05-11 セッション: AI 連携機能 (Unit-05)

### セッション概要
- **目的**: vitanota の AI 連携を深め、現場で使ってもらえる機能を探る
- **背景**: 2026-05-09 PAM auth 解決でローンチ後の重大バグを潰し終え、機能拡張フェーズへ
- **戦略**: 第一弾は「チャット → タスク + 日誌ネタ抽出」(生死線 A・B を 1 機能で同時に救う)

### 戦略レベル決定 (chimo と合意済み)
- **AI 連携第一弾**: チャット → タスク + 日誌ネタ抽出
- **AI 出力の宛先**: 本人だけ (踏み絵最安全、観測者原則)
- **mood の扱い**: AI 不可侵 (教員選択を死守)
- **A. チャット欄の置き場**: フローティング型 (画面右下バブル → モーダル/シート)
- **B. 確認 UI**: インラインバブル型 + 未承認候補別パネル
- **C. モバイル比重**: モバイルファースト (PC はレスポンシブ拡張)
- **D. チャット履歴**: 変換時スニペット保存 + 未変換揮発
- **後続機能優先順位**: リマインド → (日誌記録量が上がったら) 週次ふりかえり → 抱えすぎ

### 現状診断
- 日誌記録量がスカスカ (chimo 確認) → 生死線 A (日誌が書かれ続けるか) が破綻
- vitanota の哲学 (転勤・児童入れ替わりで失われるナレッジを副産物として残す装置) が空回り
- チャット抽出で日誌入力フリクションを解消し、生死線 A を救う

### ユニット構成
- **Unit-05: AI 連携** (新規ユニット、独立化)
  - 将来のリマインド・週次ふりかえり・抱えすぎも同 Unit に集約
  - AI 連携スタック (プロンプト管理 / モデル選定 / コスト計測) を一箇所で管理

### ワークフロープラン (アダプティブ短縮、5/2 セッション踏襲)
- ワークスペース検出: ✅ 完了 (state 既存・ブラウンフィールド継続)
- リバースエンジニアリング: ⏭️ スキップ (既存基盤理解済み)
- 要件分析: 🔵 標準で実施 (チャット抽出機能のスコープ詰め)
- ユーザーストーリー: 🔵 実施 (教員向け新機能、シーン別)
- ワークフロープランニング: 🔵 実施
- アプリケーション設計: 🔵 実施 (新コンポーネント + 抽出プロンプト + DB スキーマ)
- ユニット生成: ⏭️ スキップ (Unit-05 として直接実施)
- コンストラクション (Unit-05):
  - 機能設計: 🔵
  - NFR 要件 / NFR 設計: 判断 (AI コスト / レイテンシ要件あり)
  - インフラ設計: 🔵 (新 Lambda or Bedrock 経由)
  - コード生成: 🔵
- ビルド & テスト: 🔵 実施 (Unit-05 単独)

### 保留事項
- 5/2 セッション「ビルド・統合テスト」未完項目: 今回は触らない (chimo 指示)。post-MVP backlog で別途扱う想定

### ステージ進捗
- [x] ワークスペース検出 — 完了 (2026-05-11)
- [x] 要件分析 — 完了 (2026-05-11)
- [x] ユーザーストーリー — 完了 (2026-05-11)
- [x] ワークフロープランニング — 完了 (2026-05-11)
- [x] アプリケーション設計 — 完了 (2026-05-11)
- [x] Unit-05 機能設計 — 完了 (2026-05-11)
- [x] Unit-05 NFR 要件 — 完了 (2026-05-11)
- [x] Unit-05 NFR 設計 — 完了 (2026-05-11)
- [x] Unit-05 インフラ設計 — 完了 (2026-05-11)
- [ ] Unit-05 コード生成 — パート 1 (プランニング) 完了 (2026-05-11)、パート 2 (生成) 承認待ち
- [ ] ビルド & テスト
