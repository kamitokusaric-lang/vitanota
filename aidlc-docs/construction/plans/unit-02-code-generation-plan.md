...
- [x] Suite 3 (Session leakage) - session-leakage.test.ts (5 tests)
- [x] Suite 4 (RLS fail-safe) - session-leakage.test.ts 後半 (5 tests)
- [x] Suite 5 (is_public leak prevention) - is-public-leak.test.ts (6 tests)
- [x] Suite 6 (IDOR prevention) - idor.test.ts (6 tests)
- [x] Suite 7 (Database session strategy) - session-strategy.test.ts (6 tests)
- [x] Suite 7a (Tenant 作成時シード) - session-strategy.test.ts 後半 (6 tests)
- 注: ローカル実行は DATABASE_URL を別途設定する必要あり、CI が主要実行環境
- 統合テスト総数: 44 tests (Suite 1-2 の 10 tests + 残り 34 tests)
...
### Step 19: デプロイ成果物
- [x] `Dockerfile` は Unit-01 から変更なし（multi-stage + non-root + standalone 出力で完成済み）
- [x] `aidlc-docs/construction/unit-02/code/deployment-steps.md` 作成
  - Phase 1 手動運用手順 (Step A: dev / Step B: prod、12 ステップ)
  - Phase 2 CI/CD 自動化への移行手順
  - App Runner / マイグレーション両方のロールバック手順
- [x] `scripts/db-migrator/` Lambda マイグレーター参考実装
  - `handler.ts`: migrate / status / drop コマンド対応・IAM auth token・_migrations 履歴管理
  - `package.json`: pg + @aws-sdk/rds-signer のみの軽量依存
  - `tsconfig.json`: Node.js 20 ARM64 ランタイム向け
  - `README.md`: ビルド・デプロイ・運用手順・3 ロール権限分離・トラブルシューティング
- [x] `.gitignore` に Lambda ビルド成果物 (`function.zip`) を追加
