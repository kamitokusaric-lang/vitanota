...
- [ ] `pages/api/system/tenants.ts` に `tagRepository.seedSystemDefaults` 呼び出しを追加
- [ ] テナント作成と同一トランザクション内で 8件のデフォルトタグを INSERT
- [ ] NFR-U02-03 対応
...
- [ ] `src/features/journal/components/EntryForm.tsx` - 作成・編集フォーム
  - React Hook Form + zodResolver
  - TagFilter 統合
  - 200文字カウンター
  - 公開設定トグル
  - data-testid 属性
- [ ] `src/features/journal/components/TagFilter.tsx` - PP-U02-01 useMemo + includes
- [ ] `src/features/journal/components/TimelineList.tsx` - SWR による共有タイムライン表示
- [ ] `src/features/journal/components/EntryCard.tsx` - エントリ1件の表示
- [ ] `src/features/journal/components/MyJournalList.tsx` - マイ記録表示
- [ ] `pages/journal/index.tsx` - タイムラインページ
- [ ] `pages/journal/mine.tsx` - マイ記録ページ
- [ ] `pages/journal/new.tsx` - 新規作成ページ
- [ ] `pages/journal/[id]/edit.tsx` - 編集ページ
...
### Step 11: ログイベント定義
- [ ] `src/shared/lib/logger.ts` に Unit-02 イベント定義を追加
  - `journal_entry_created` / `updated` / `deleted` / `read`
  - `journal_entry_list_read`
  - `tag_created` / `deleted` / `list_read`
  - `session_created` / `revoked` / `expired`
- [ ] 本文を redact フィールドに追加
