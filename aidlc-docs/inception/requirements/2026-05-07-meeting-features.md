...
- **AC-A-05**: 「メールアドレスを改行 or カンマ区切りで貼り付け」→「全員 teacher として一括招待」ボタンで複数招待を 1 操作で発行
- **AC-A-06**: 一括投入結果がそのまま下の一覧に反映 (再リロード不要)
- **AC-A-07**: 期限切れ招待には「再発行」ボタン (bulk API へ 1 件投げ、既存重複ロジックで古いトークン無効化)
...
- 一括投入時のロール選択 UI (MVP は teacher 固定、ロール別招待が必要なら個別 API で対応)
