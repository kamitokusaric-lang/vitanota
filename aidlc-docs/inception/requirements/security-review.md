...
本ドキュメントの「全体アーキテクチャ図」「AWS リソース一覧」は **Phase 2（本格稼働）** を想定した**フル設計**である。
MVP ローンチ時点では **Phase 1（簡略構成）** が採用されており、一部のリソースは Phase 2 で追加される。

フェーズの詳細と段階的移行計画は `aidlc-docs/construction/deployment-phases.md` を参照。

**Phase 1（MVP）の主な簡略点**:
- RDS Proxy なし・RDS 単一 AZ・環境分離なし・Lambda 監視なし・アラーム 5 個・Object Lock 90 日
- 月額: 約 ¥6,000-8,000（Phase 2 は ¥23,000-26,000）

**セキュリティ論点は全て Phase 1 でも対応済み** または **対応計画あり**。詳細は `deployment-phases.md` の「論点・リスクとの紐づき」セクションを参照。
