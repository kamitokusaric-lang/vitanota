...
- **2026-06-03 update (Next.js 15 + React 19 を実施)**: framework 部分を別 PR `feature/2026-06-03-next15-react19` で実施。next 15.5.19 / react 19.2.7 / react-dom 19.2.7 / @types/react 19 / eslint-config-next 15.5.19 / @testing-library/react 16。**Pages Router のみ**のため App Router 系 breaking (async request API / fetch caching 等) は無関係、ソース変更ゼロ (codemod 不要、production code 不変) で type-check / build / lint / test 391 pass / dev 起動+signin SSR を確認。
