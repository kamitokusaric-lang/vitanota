...
> **✅ 2026-04-21 実装完了**: 本書記載の Lambda Proxy + PKCE 認証フローは稼働中。
> DB 接続も IAM 認証に切り替え完了。本書は当時の状況スナップショットとして履歴保存。
> 現行の招待フロー仕様は `aidlc-docs/construction/user-onboarding-flow.md` 参照。
