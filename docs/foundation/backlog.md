...
- **2026-06-03 update (vitest 4 upgrade による再校正)**: vitest 1.6.1 → 4.1.8 で v8 coverage が AST-aware remapping に変わり、 branch / function を厳密にカウントするようになった (分母: branches 543→1307 / functions 211→579)。 テスト内容は不変だが計測基準が変わり数値が下がったため閾値を実測フロア直下に置き直した:
  - branches: 75 → **40** (v4 計測値 41.54、 旧 v1 計測 77.41)
  - functions: 55 → **35** (v4 計測値 36.44、 旧 v1 計測 56.43)
  - lines/statements: **40** 据え置き (v4 計測 43.14 / 41.32 で旧と概ね同水準)
  - ⚠️ 元の目標 80/70/80/80 は **v1 計測基準**。 v4 では同じテスト網羅率でも数値が異なるため、 引き上げ目標は v4 基準で再設定が必要 (旧目標値をそのまま適用しない)。
...
### ⚪ AI 複数人格が公開投稿に食いつく (「うるさいインターネット」)
- **発見日**: 2026-07-01 (ふりかえり→AIリコメンド UX 詰めセッション)
- **背景**: 公開した職員室ノート投稿に、**固定の常連 AI キャラ数人**がそれぞれ違う角度で反応し、賑やかで「また来たい」場にする。参照= chimo 提示の「うるさいインターネット」(投稿に AI が左右から食いつく)。
- **叩き台キャスト (レンズ=引き出したい価値)**: ホリ=やり方/手順を掘る(ナレッジ発掘) / ナギ=自分の経験を重ねて横につなぐ / ナゼ=素朴な疑問で暗黙知を言語化 / (補欠)トキ=後日そっと続きを気にする。名前は仮。
- **踏み絵 (これを外すと即死)**:
  - **公開側にだけ棲む**。マイノート(ふりかえり=私的)には絶対入れない (観測されると壊れる)。「つぶやいた後」の世界にだけ。
  - **褒めない**。全員"好奇心"の register のみ (「教えてほしい」「コツ知りたい」)。「素敵/えらい/すごい」等の評価語禁止 ([[feedback_ai_output_guards]])。多重の空拍手は最悪。
  - **人間のフリをしない**。既存「vitanota AI」カードの延長で専用レーン/バッジ。本物の同僚と混ざらない。
  - **人間を食わない**。1投稿に出るのは 1〜2 人・中身に合う奴だけ。主役は先生同士の 3 リアクション。AI は"無反応で循環が止まる"(階層E)を埋める下地。
  - 感情代弁 (「大変だったね」) は踏み絵。「その後どう?」等の続きを気にする温度に留める。
- **効用**: 好奇心の食いつきが暗黙知を引き出す → vitanota の why (転勤で失われるナレッジを残す) に直結。単なる賑やかしでなく機能を持つ。
- **関連**: [journal/retro-recommend.md](../features/journal/retro-recommend.md) (公開/私的の非対称の延長) / PHILOSOPHY §4 踏み絵
...
- **graduate (AI 生成へ) の条件**: 明示的な §7 再決定。かつ以下の制約を満たす再設計:
  ...
  - **教員無記名** (誰の投稿かを要約に出さない)
  - **fact + 提案のみ** (感情代弁・励まし・評価・採点なし。PHILOSOPHY §4.3 / [[feedback_ai_output_guards]])
  - **mood・生徒感情は要約しない** ([[feedback_mood_ai_untouchable]])
  - **生徒個人を特定しない** (chimo ガードレール「生徒情報の特定」)
- **関連**: 下記「先週のvitanotaレポート 機能 (凍結)」と相互参照 / 接続経路は同項目の調査結果を流用可
