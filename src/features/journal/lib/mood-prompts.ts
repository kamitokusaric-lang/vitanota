...
// MoodPromptBar の各 mood アイコンの上に 1 文字ずつ吹き出しで散らす用、
// ピッタリ 5 文字前後 (Array.from でカウント) の短い挨拶セット。時間帯別 (chimo 提示)。
const MORNING_GREETINGS_5 = [
  'おはよう！ってつぶやいてもいいんだよ',
  'どんな1日になりそう？一発つぶやいてね！',
  'ねむいよー、ってつぶやきたい',
] as const;

const NOON_GREETINGS_5 = [
  'おつかれ！思いのたけをつぶやくのだ！',
  'ひとやすみしながら、みんなのつぶやきを眺めよう',
  '頑張ってるアピールをつぶやいて！',
] as const;

const EVENING_GREETINGS_5 = [
  '今日の出来事を日誌に書いてね！あとでラクだから！',
  'みんなに共有したいナレッジがあったはず、書いて！',
  '日誌を書いて帰る！',
] as const;

export function pickRandomFiveCharGreeting(date?: Date): string {
  const tod = getTimeOfDay(date);
  const list =
    tod === 'morning'
      ? MORNING_GREETINGS_5
